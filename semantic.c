...
int Connection(char *p ){
	...
	if (p != NULL) {
		...
		if (strncmp(node->value, "close",5)==0 ) res=1;
		tok=tok->next;
		}
...
int lecturefich(char *monFichier, char* montype, int len){
	...
	{
		if(strncmp(montype,chaine,len) == 0){
			return(1);
		}
...
int mediarange(char* p, char* monfichier){
	_Token *r,*tok; Lnode *node;
	Lnode *root;
			char * copie = malloc(MAX*sizeof(char));
	int faux = 400;
	root=getRootTree();
	int oui=0;
	int ra;
...
int Verification(){
	...
	if( (ver=contentlenght("Content_Length")) != 200 && (ver!=1)){return(ver);}
	if( (conn=Connection("connection_option")) != 200  && (conn !=1) ){return(conn);}
	if( (ver=TransferEnco("Transfer_Encoding")) != 200 && (ver !=2)){return(ver);}
	if( (ver=mediarange("media_range","accept-types.txt")) !=200){return(ver);}
	/* Test si les header ne sont pas en double >>> Bad Request*/
	if( (conn=co("Connection")) != 200) return(ver);
